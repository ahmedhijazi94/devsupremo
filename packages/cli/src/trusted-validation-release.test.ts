...
const fixtureBytes = readFileSync(new URL('../../../src/lib/github/fixtures/validation-files-4.0.5.json.gz', import.meta.url))
const files = JSON.parse(gunzipSync(fixtureBytes).toString('utf8')) as ArchivedFiles
const metadata = JSON.parse(gunzipSync(readFileSync(new URL('../../../src/lib/github/fixtures/validation-policy-4.0.5.json.gz', import.meta.url))).toString('utf8')) as ArchivedMetadata
...
describe('current engine verifies immutable 4.0.5 project rails', () => {
  it('pins the archived validator bytes to the published source commit', () => {
    expect(files).toMatchObject({ templateVersion: '4.0.5', sourceCommit: 'f4acc40aa55bee2371cfc4b9a3e8aa8c9c8504cc' })
    expect(createHash('sha256').update(fixtureBytes).digest('hex'))
      .toBe('47c794cbcd88c25fb0dc2950c455d13180d654aec9b7aa21bcbd14c21a781453')
  })
