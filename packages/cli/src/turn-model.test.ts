...
describe('explicit executable state and behavior evidence', () => {
  ...
  it('health is fail-closed while keeping the edit loop independent', () => {
    ...
    expect(deriveProjectHealth({ ...input, activeTurn: true, securityState: 'unsafe', recovery: pending({ attempts: 3, status: 'needs_human_attention' }) })).toBe('developing')
    expect(deriveProjectHealth({ ...input, activeTurn: true, recovery: repairing(), managedRepair: true })).toBe('repairing')
    expect(deriveProjectHealth({ ...input, activeTurn: true, securityState: 'unsafe', remoteStatus: 'invalid' })).toBe('blocked')
    expect(deriveProjectHealth({ ...input, activeTurn: true, securityState: 'unsafe', workspace: { ...workspace, environment: 'production' } })).toBe('blocked')
