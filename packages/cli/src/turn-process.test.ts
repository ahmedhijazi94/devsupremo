...
main().catch(e=>{console.error(e.message);process.exitCode=1});`)
    ...
    expect(cold.state?.turn.phase).toBe('work')
    expect(cold.state?.turn.recovery?.checkpointId).toBe(failed.checkpointId)
    expect(cold.state?.turn.recovery?.attempts).toBe(0)
    // Explicit repair remains available without being forced by the date request.
    expect((await call('repair-start')).allowed).toBe(true)
    ...
    expect((await call('complete')).allowed).toBe(true)
    await call('worker')
    expect(queue().at(-1)?.validationStatus).toBe('deferred')
