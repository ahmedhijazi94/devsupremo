...
import { readBrowserDiagnostics } from './browser-diagnostics'
...
export function turnAgentResponse(output: TurnResult, cwd?: string) {
  ...
  const browserDiagnostics = cwd && state ? readBrowserDiagnostics(cwd, state.turn.projectId) : null
  return {
