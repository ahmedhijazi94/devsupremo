...
import { dismissSecretSchema, saveSecretSchema, safeSecretError } from '@/lib/secret-requests/policy'
...
import type { SecretRequestView } from '@/lib/secret-requests/policy'
export type { SecretRequestView } from '@/lib/secret-requests/policy'

export async function getSecretRequests(projectId: string): Promise<{ requests?: SecretRequestView[]; error?: string }> {
  ...
  } catch (error) { return { error: safeSecretError(error) } }
}

/** Only the project owner can submit a value, through this form action. The destination is loaded by request ID. */
export async function saveSecret(input: { projectId: string; requestId: string; value: string }): Promise<{ ok?: true; error?: string }> {
...
export async function dismissSecretRequest(input: { projectId: string; requestId: string }): Promise<{ ok?: true; error?: string }> {
