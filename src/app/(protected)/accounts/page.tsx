  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      <Card className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted text-xs">
                Para criar e gerenciar repositórios
              </p>
            </div>
          </div>
          <ConnectGithubButton />
        ...
        {githubAccounts.length === 0 ? (
          <div className="space-y-2 rounded-[var(--radius-inner)] p-10 text-center">
            ...
            <p className="text-muted mx-auto max-w-xs text-xs">
              O Supremo não poderá criar ou modificar repositórios até você
              conectar uma conta.
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-center gap-3">
                  {acc.avatar_url ? (
                    ...
                    <div className="bg-sunken h-10 w-10 rounded-full" />
                  )}
                  <div>
                    <p className="text-sm font-medium">
                      {acc.login}
                    ...
                    <p className="text-muted text-xs">
                      Autorizada {formatRelativeTime(acc.created_at)} · escopos:{' '}
                      {acc.scopes?.join(', ') || 'repo'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <HealthBadge
                    health={health.get(`github:${acc.id}`) ?? 'unknown'}
                  />
      ...
      <Card className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-[var(--radius-control)] border border-[#3ECF8E]/20 bg-[#3ECF8E]/10">
              <Database className="h-5 w-5 text-[#3ECF8E]" />
            </div>
            <div>
              <h2 className="text-base font-semibold">Supabase</h2>
              <p className="text-muted text-xs">
                Para provisionar bancos e aplicar migrations
              </p>
            </div>
          </div>
        ...
        {supabaseAccounts.length === 0 ? (
          <div className="space-y-2 rounded-[var(--radius-inner)] p-10 text-center">
            ...
            <p className="text-muted mx-auto max-w-xs text-xs">
              O Supremo não poderá provisionar bancos de dados automáticamente
              sem um Access Token.
            </p>
          </div>
        ) : (
            ...
            {supabaseAccounts.map((acc) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium">{acc.org_name}</p>
                    <p className="text-muted text-xs">
                ...
                <div className="flex items-center gap-3">
                  <HealthBadge
                    health={health.get(`supabase:${acc.id}`) ?? 'unknown'}
                  />
