    ...
    <div className="space-y-3 sm:space-y-4">
      ...
      <Card>
        ...
        <CardNote className="mt-1">
          As contas que o Supremo usa para criar e modificar os seus projetos.
        </CardNote>
      </Card>
      ...
      <Card className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-foreground/5 border border-line flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-xs text-muted">Para criar e gerenciar repositórios</p>
            </div>
          </div>
          <ConnectGithubButton />
        </div>
        ...
        {githubAccounts.length === 0 ? (
          <div className="rounded-xl border border-line border-dashed p-10 text-center space-y-2">
          ...
          <div className="grid gap-3">
            {githubAccounts.map((acc) => (
              <div key={acc.id} className="flex items-center justify-between p-4 rounded-xl border border-line bg-surface">
                <div className="flex items-center gap-3">
                  {acc.avatar_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={acc.avatar_url} alt="" className="w-10 h-10 rounded-full" />
                  ) : (
                    <div className="w-10 h-10 rounded-full bg-sunken" />
                  )}
                  <div>
                    <p className="font-medium text-sm">
                      {acc.login}
                      {acc.name ? <span className="text-muted font-normal ml-1">({acc.name})</span> : null}
                    </p>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted">Para provisionar bancos e aplicar migrations</p>
            </div>
          </div>
        ...
        {vercelAccounts.length === 0 ? (
          ...
          <div className="grid gap-3">
            {vercelAccounts.map((acc) => (
              <div key={acc.id} className="flex items-center justify-between p-4 rounded-xl border border-line bg-surface">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-foreground/5 border border-line flex items-center justify-center">
                    <svg className="w-4 h-4" viewBox="0 0 76 65" fill="currentColor" aria-hidden>
                      <path d="M37.59.25l36.95 64H.64l36.95-64z" />
                    </svg>
                  </div>
