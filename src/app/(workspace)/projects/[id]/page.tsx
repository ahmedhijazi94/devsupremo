...
export default async function ProjectPage({
  ...
  return (
    ...
    <div className="flex h-screen flex-col gap-3 p-3 sm:gap-4 sm:p-4">
      {/* Cabeçalho */}
      <header className="bg-surface flex shrink-0 items-center gap-4 rounded-[var(--radius-card)] px-4 py-3">
        <Link
          href="/dashboard"
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-3 sm:gap-4 lg:flex-row">
        {/* Painel lateral */}
        <aside className="bg-surface flex w-full shrink-0 flex-col gap-4 overflow-y-auto rounded-[var(--radius-card)] p-4 lg:w-[360px]">
          {!provisioned && (
            <ProvisionCard
              projectId={project.id}
