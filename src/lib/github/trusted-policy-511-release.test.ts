...
const fixtureBytes = readFileSync(new URL('./fixtures/validation-files-4.0.13-5.1.1.json.gz', import.meta.url))
...
function materialize(project: ArchivedProject): string {
  const cwd = mkdtempSync(join(tmpdir(), 'supremo-release-511-'))
  workspaces.push(cwd)
...
describe('release 5.1.1 remains authorized after engine upgrades', () => {
  it('pins the actual release files rather than regenerating them from the current scaffold', () => {
    expect(fixture.sourceCommit).toBe('630cc8189629dff140c40abc8df13ac5c85884a5')
    expect(createHash('sha256').update(fixtureBytes).digest('hex')).toBe('a393ffd63046c2574bc0d742ae77f9fcbe2a87da58d550c394a72752e3fa088a')
    expect(fixture.projects.map(project => `${project.version}/${project.kind}`)).toEqual([
      '4.0.13/public', '4.0.13/solo', '4.0.13/team', '5.1.1/public', '5.1.1/solo', '5.1.1/team',
    ])
  })
