...
describe('banco preparado antes de finalizar provisionamento', () => {
  it('não aceita falha de migration como sucesso e não cria outro banco no retry', async () => {
    ...
    const pending = expect(provisionSupabase(client, 'user', 'account', 'app', files, { existingRef: 'existing' })).rejects.toThrow(/falha na migration/)
  ...
  it('banco que não ficou saudável permanece retomável, sem sucesso falso', async () => {
    ...
    const pending = expect(provisionSupabase(client, 'user', 'account', 'app', files, { existingRef: 'existing' })).rejects.toThrow(/ainda em preparação/)
  ...
  it('aplica todas as migrations em ordem e só então retorna sucesso', async () => {
    ...
    const pending = provisionSupabase(client, 'user', 'account', 'app', files, { existingRef: 'existing' })
    await vi.advanceTimersByTimeAsync(5000)
