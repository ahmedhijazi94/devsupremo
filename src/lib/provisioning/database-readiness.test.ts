...
describe('banco preparado antes de finalizar provisionamento', () => {
  it('não aceita falha de migration como sucesso e não cria outro banco no retry', async () => {
    ...
    const pending = expect(provisionSupabase(client, 'user', 'account', 'app', 'team', { existingRef: 'existing', verifyDevelopment: async () => {} })).rejects.toThrow(/falha na migration/)
  ...
  it('banco que não ficou saudável permanece retomável, sem sucesso falso', async () => {
    ...
    const pending = expect(provisionSupabase(client, 'user', 'account', 'app', 'team', { existingRef: 'existing', verifyDevelopment: async () => {} })).rejects.toThrow(/ainda em preparação/)
  ...
  it('aplica todas as migrations em ordem e só então retorna sucesso', async () => {
    ...
    const pending = provisionSupabase(client, 'user', 'account', 'app', 'team', { existingRef: 'existing', verifyDevelopment: async () => {} })
    await vi.advanceTimersByTimeAsync(5000)
    expect((await pending).projectRef).toBe('existing')
    expect(sql).toHaveLength(2)
    expect(sql[0]).toContain('CREATE TABLE IF NOT EXISTS memberships (')
    expect(sql[1]).toContain('DROP POLICY IF EXISTS "memberships_insert_own" ON public.memberships;')
  })
})
...
it('retry sem autoridade de ambiente nunca chega ao provedor', async () => {
  ...
  await expect(provisionSupabase(client, 'user', 'account', 'app', 'team', { existingRef: 'unknown' })).rejects.toThrow(/classificação development/)
