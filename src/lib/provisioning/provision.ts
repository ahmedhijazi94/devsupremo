...
/**
 * Core de provisioning — reutilizado pelo action `scaffoldProject` (wrapper fino)
 * e pelo E2E script-driven. Recebe userId + client Supabase explícitos (sem
 * requireUser), pra rodar tanto no runtime do Next quanto num script server-side
 * com service_role. Não chama revalidatePath: quem chama (a action) revalida.
 */
export async function provisionProject(params: {
  projectId: string
  userId: string
