...
function read(cache: unknown, queue = [{ projectId, checkpointId }], lifecycle: unknown = {}) {
...
describe('generated preflight recovery', () => {
  ...
  it('keeps ordinary failures advisory without pretending validation passed', () => {
    ...
    expect(result).toMatchObject({ developmentPolicy: { validation: 'background_adaptive', previousFailures: 'advisory' } })
  ...
  it('respects an explicit on-request validation override without hiding previous diagnostics', () => {
    expect(read({ current: failed() }, undefined, { validation_mode: 'on_request' })).toMatchObject({
      developmentPolicy: { validation: 'on_request', previousFailures: 'advisory' },
      action: 'continue_with_diagnostics',
    })
  })
  it('permits development fixes for security/infrastructure while preserving execution and integration authority', () => {
    for (const category of ['security', 'infrastructure']) {
      ...
      expect(result).toMatchObject({ action: 'continue_with_diagnostics', stale: true, developmentPolicy: { previousFailures: 'advisory' } })
