...
describe('framework selection is immutable after creation', () => {
  it('selects Start for new projects and respects an explicit future-creation rollback', () => {
    ...
    expect(newProjectTemplateVersion()).toBe('5.0.0')
    vi.stubEnv('SUPREMO_NEW_PROJECT_STACK', 'nextjs')
    expect(newProjectTemplateVersion()).toBe('4.0.10')
  })
  ...
  it('rollback changes only future insertions, not an already selected template', () => {
    ...
    expect(newProjectTemplateVersion('nextjs')).toBe('4.0.10')
    expect(provisioningTemplate(pinned)).toEqual({ stack: 'tanstack-start-vite', version: '5.0.0' })
    expect(provisioningTemplate(null)).toEqual({ stack: 'nextjs', version: '4.0.10' })
    expect(latestTemplateVersion('4.0.8')).toBe('4.0.10')
    expect(latestTemplateVersion('5.0.0')).toBe('5.0.0')
  ...
  it('requires version, declaration and installed framework evidence to agree', () => {
    ...
    expect(latestTemplateVersion(undefined)).toBe('4.0.10')
  })
})
